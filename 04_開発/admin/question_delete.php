...
<div class="sticky-top">
    ...
    <?php
        ...
        $sql = "DELETE FROM quizcategorys WHERE question_id = ?";
            $ps = $pdo -> prepare($sql);
            $ps->bindValue(1,$_POST['id'], PDO::PARAM_INT);
            $ps -> execute();
        $sql1 = "DELETE FROM quiz WHERE question_id = ?";
            $ps1 = $pdo -> prepare($sql1);
            $ps1->bindValue(1,$_POST['id'], PDO::PARAM_INT);
            $ps1 -> execute();
    ?>
<h2 class="text-center mt-5">問題が削除されました。</h2>
